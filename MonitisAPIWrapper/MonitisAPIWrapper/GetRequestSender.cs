...
{
    /// <summary>
    /// Sends a GET web request to the Monitis API
    /// </summary>
    /// <typeparam name="T">The type of web response</typeparam>
    public class GetRequestSender<T> : IRequestSender<T>
    {
        ...
        public GetRequestSender(string url, string action, string apikey, ChecksumCalculator checksumCalculator, AuthenticationTokenGetter<AuthenticationTokenResponse> authTokenGetter, int version, ILog log)
        {
            URL = url;
            Action = action;
            Apikey = apikey;
        ...
        private string GetAuthToken()
        {
            ...
            var response = AuthTokenGetter.ResponseData as AuthenticationTokenResponse;
        ...
        public void Send()
        {
            string queryStringData = AggregateAllPostData();

            var request = WebRequest.Create(URL + "?" + queryStringData);
            request.Method = "GET";
        ...
        private string AggregateAllPostData()
        {
        ...
        private static string EncodePostData(Dictionary<string, object> postData)
        {
