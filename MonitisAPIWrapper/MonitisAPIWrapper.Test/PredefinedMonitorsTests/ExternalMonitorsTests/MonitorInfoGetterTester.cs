...
{
    [TestFixture]
    public class MonitorInfoGetterTester : InjectedTest
      //  ExternalMonitorDependentTest
    {
        [Test]
        public void CanGetMonitorInfo()
        {
            var getter = Container.Resolve<ExternalMonitorInfoGetter>();
            getter.RequestSender.Validation = Validation.HMACSHA1;
            getter.TestId = 39333;
            
            Assert.DoesNotThrow(getter.Get);
            Assert.IsTrue(getter.Success);
            Assert.IsNotNull(getter.Result);
        }
    }
}
